...
struct button_state {
    ...
    ImVec4 text = ImVec4(1, 1, 1, 0);
    float alpha = 0.0f; // dynamic alpha
};
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(
        ...
        IsItemActive() ? ui::colors::button::background_light : ui::colors::button::background,
        g.IO.DeltaTime * 6.f
    );

    it_anim->second.text = ImLerp(
        it_anim->second.text,
        IsItemHovered() ? ui::colors::loader::main : ui::colors::loader::text_disabled,
        g.IO.DeltaTime * 6.f
    );

    ImDrawList* dl = ImGui::GetWindowDrawList();
    dl->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 6.0f);
    dl->AddRect(bb.Min, bb.Max, GetColorU32(ui::colors::loader::outline_white), 6.0f);
    ...
    dl->AddText(
        ...
        GetColorU32(IM_COL32(255,255,255,255)),
        label
    );
...
bool ImGui::ButtonIconEx(const char* label, const char* icon, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ImGui::PushFont(font.buttons_icon);
    ImVec2 icon_size = CalcTextSize(icon, NULL, true);
    ImGui::PopFont();

    float spacing = 6.0f;
    ImVec2 content_size(icon_size.x + spacing + label_size.x, ImMax(icon_size.y, label_size.y));

    ImVec2 size = CalcItemSize(size_arg, content_size.x + style.FramePadding.x * 2, content_size.y + style.FramePadding.y * 2);
    const ImRect bb(pos, pos + size);
    ...
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
        it_anim = anim.insert({ id, button_state() }).first;

    // --- background animation ---
    ImVec4 bg_target = IsItemActive()
        ? ui::colors::button::background_light
        : ui::colors::button::background;

    it_anim->second.background = ImLerp(it_anim->second.background, bg_target, g.IO.DeltaTime * 6.f);

    // --- text animation ---
    ImVec4 text_target = IsItemHovered()
        ? ui::colors::loader::main
        : ui::colors::loader::text_disabled;

    it_anim->second.text = ImLerp(it_anim->second.text, text_target, g.IO.DeltaTime * 6.f);

    // --- alpha animation ---
    float target_alpha = 0.65f; // idle
    if (hovered) target_alpha = 1.0f;
    if (held)    target_alpha = 0.85f;

    it_anim->second.alpha = ImLerp(it_anim->second.alpha, target_alpha, g.IO.DeltaTime * 8.f);

    // --- draw background ---
    ImDrawList* dl = ImGui::GetWindowDrawList();

    ImU32 bg_col = GetColorU32(ImVec4(
        it_anim->second.background.x,
        it_anim->second.background.y,
        it_anim->second.background.z,
        it_anim->second.alpha
    ));
    ...
    // --- text + icon ---
    float total_w = icon_size.x + spacing + label_size.x;
    float start_x = bb.Min.x + (size.x - total_w) * 0.5f;
    float center_y = bb.Min.y + (size.y - content_size.y) * 0.5f;

    ImU32 text_col = GetColorU32(ImVec4(1,1,1,
        it_anim->second.alpha
    ));
    ...
    ImGui::PushFont(font.buttons_icon);
    dl->AddText(
        ImVec2(start_x, center_y + (content_size.y - icon_size.y) * 0.5f),
        text_col,
        icon
    );
    ImGui::PopFont();
    ...
    ImGui::PushFont(font.montserrat_semibold[3]);
    dl->AddText(
        ImVec2(start_x + icon_size.x + spacing, center_y + (content_size.y - label_size.y) * 0.5f),
        text_col,
        label
    );
    ImGui::PopFont();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const ImU32 grab_col = GetColorU32(ui::colors::loader::background);
...
static std::string TruncateTextEllipsis(ImFont* font, float font_size, float max_width, const char* text)
{
    std::string result;
    const char* ellipsis = "..";
    float ellipsis_width = font->CalcTextSizeA(font_size, FLT_MAX, 0.0f, ellipsis).x;

    for (int i = 0; text[i] != '\0'; ++i)
    {
        result += text[i];
        float width = font->CalcTextSizeA(font_size, FLT_MAX, 0.0f, result.c_str()).x;

        if (width + ellipsis_width > max_width)
        {
            result.pop_back(); // sacar el último que se pasó
            result += ellipsis;
            break;
        }
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float spacing = 6.f; // espacio vertical entre label y combo

    ImVec2 cursor = window->DC.CursorPos;

    // Label arriba
    ImVec2 label_size = CalcTextSize(label);
    ImRect label_bb(cursor, cursor + ImVec2(full_width, label_size.y));

    // Combo abajo del label
    ImRect combo_bb(
        ImVec2(cursor.x, label_bb.Max.y + spacing),
        ImVec2(cursor.x + full_width, label_bb.Max.y + spacing + height)
    );

    ImRect total_bb(label_bb.Min, combo_bb.Max);
    ...
    auto& a = anim[id];
    a.text = ImLerp(a.text, popup_open ? ui::colors::loader::text : hovered ? ui::colors::loader::text : ui::colors::loader::text_disabled, g.IO.DeltaTime * 6.f);
    a.combo = ImLerp(a.combo, popup_open ? ui::colors::loader::main : hovered ? ui::colors::loader::main : ui::colors::checkbox::circle_disabled, g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(combo_bb.Min, combo_bb.Max, GetColorU32(ui::colors::loader::background), 4);
    window->DrawList->AddRect(combo_bb.Min, combo_bb.Max, GetColorU32(ui::colors::loader::outline_white), 4);

    const float preview_padding_x = 8.0f;
    ImVec2 preview_pos = combo_bb.Min + ImVec2(preview_padding_x, (height - ImGui::GetFontSize()) * 0.5f - 1);
    float max_preview_width = combo_bb.Max.x - preview_pos.x - 4.0f;

    PushFont(font.montserrat_semibold[3]);

    std::string display_value_str = preview_value;
    if (CalcTextSize(preview_value).x > max_preview_width)
        display_value_str = TruncateTextEllipsis(g.Font, g.FontSize, max_preview_width, preview_value);

    window->DrawList->AddText(preview_pos, GetColorU32(ui::colors::loader::text), display_value_str.c_str());

    ImVec2 label_text_pos = label_bb.Min;
    window->DrawList->AddText(label_text_pos, GetColorU32(a.text), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect)) {
                        ...
                        ImVec2 adjusted_min = ImVec2(rect.Min.x, rect.Min.y + y_offset);
                        ImVec2 adjusted_max = ImVec2(rect.Max.x, rect.Max.y + y_offset + 1);

                        draw_window->DrawList->AddRectFilled(adjusted_min, adjusted_max, bg_color);
                    }
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if (!is_password) {
                draw_window->DrawList->AddText(
                    g.Font,
                    g.FontSize,
                    draw_pos - draw_scroll + ImVec2(0, -1),
                    col,
                    buf_display,
                    buf_display_end,
                    0.0f,
                ...
                draw_window->DrawList->AddText(
                    g.Font,
                    g.FontSize,
                    draw_pos - draw_scroll + ImVec2(0, 7),
                    col,
                    buf_display,
                    buf_display_end,
                    0.0f,
        ...
        if (render_cursor)
        {
            ...
            std::string buf_display_str = buf_display_from_state ? std::string(state->TextA.Data) : std::string(buf);

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {

                if (buf_display_str.length() != 0) {
                    draw_window->DrawList->AddRectFilled(
                        ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5) + ImVec2(it_filing->second.value + 0, 6),
                        ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5) + ImVec2(it_filing->second.value + 1, 21),
                    ...
                    draw_window->DrawList->AddRectFilled(
                        ImVec2(frame_bb.Min.x, frame_bb.Min.y + 6) + ImVec2(it_filing->second.value + 0, 6),
                        ImVec2(frame_bb.Min.x, frame_bb.Min.y + 6) + ImVec2(it_filing->second.value + 1, 21),
                        GetColorU32(ImGuiCol_Text)
                    );
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Color animado
    ImVec4 target_color = selected ? ui::colors::loader::main : (hovered ? ui::colors::loader::text : ui::colors::loader::text_disabled);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImGui::GetColorU32(ui::colors::loader::background), style.FrameRounding);
    ...
    if (selected)
    {
        ...
        float bar_y = actual_text_pos.y + (label_size.y - bar_height) * 0.5f + 1;
